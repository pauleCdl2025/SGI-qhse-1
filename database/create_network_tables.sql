...
-- Ajouter la contrainte de clé étrangère si la table profiles existe
SET @profiles_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'profiles');
SET @sql = IF(@profiles_exists > 0,
  'ALTER TABLE network_equipment ADD CONSTRAINT fk_network_equipment_created_by FOREIGN KEY (created_by) REFERENCES profiles(id) ON DELETE RESTRICT',
  'SELECT "Table profiles non trouvée, contrainte de clé étrangère ignorée" AS message');
...
SET @profiles_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'profiles');
SET @sql = IF(@profiles_exists > 0,
  'ALTER TABLE network_subscriptions ADD CONSTRAINT fk_network_subscriptions_created_by FOREIGN KEY (created_by) REFERENCES profiles(id) ON DELETE RESTRICT',
  'SELECT "Table profiles non trouvée, contrainte de clé étrangère ignorée" AS message');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @profiles_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES 
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'profiles');
SET @sql = IF(@profiles_exists > 0,
  'ALTER TABLE network_inventory ADD CONSTRAINT fk_network_inventory_created_by FOREIGN KEY (created_by) REFERENCES profiles(id) ON DELETE RESTRICT',
  'SELECT "Table profiles non trouvée, contrainte de clé étrangère ignorée" AS message');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
