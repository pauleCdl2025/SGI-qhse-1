...
export const SuperviseurQHSEPortal = ({ user, incidents, visitors, plannedTasks, bookings, notifications, users, onNavigate, onDeleteIncident }: PortalProps) => {
  ...
  return (
    <div className="space-y-8 fade-in">
      <PortalPageHeader
        iconName="UserCog"
        title={user.role === 'assistante_qhse' ? 'Portail Assistante QHSE' : 'Portail QHSE'}
        subtitle={`${user.civility} ${user.first_name} ${user.last_name} - ${user.role === 'assistante_qhse' ? 'Assistante Qualité, Hygiène, Sécurité et Environnement' : 'Superviseur Qualité, Hygiène, Sécurité et Environnement'}`}
