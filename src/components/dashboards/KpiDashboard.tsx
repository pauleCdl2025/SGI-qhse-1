...
export const KpiDashboard = ({ incidents, biomedicalEquipment, plannedTasks, visitors, bookings, users, maintenanceTasks, currentUser }: KpiDashboardProps) => {
  ...
  const showSecurity = currentUser.role === 'agent_securite' ||
    currentUser.role === 'superviseur_agent_securite' ||
    currentUser.role === 'superviseur_qhse' ||
    currentUser.role === 'superadmin' ||
    filteredVisitors.length > 0 ||
    securityIncidents.length > 0;

  const showMaintenance = currentUser.role === 'agent_entretien' ||
    currentUser.role === 'superviseur_agent_entretien' ||
    currentUser.role === 'superviseur_qhse' ||
    currentUser.role === 'superadmin' ||
    maintenanceIncidents.length > 0 ||
    maintenanceTasksByStatus.length > 0;

  const showTechnical = currentUser.role === 'technicien' ||
    currentUser.role === 'superviseur_technicien' ||
    currentUser.role === 'superviseur_qhse' ||
    currentUser.role === 'technicien_polyvalent' ||
    currentUser.role === 'superadmin' ||
    technicalIncidents.length > 0 ||
    totalPlannedMaintenanceTasks > 0;

  const showBiomedical = currentUser.role === 'biomedical' ||
    currentUser.role === 'technicien' ||
    currentUser.role === 'superviseur_technicien' ||
    currentUser.role === 'superviseur_qhse' ||
    currentUser.role === 'superadmin' ||
    filteredBiomedicalEquipment.length > 0 ||
    maintenanceTasksByType.length > 0;

  const showPlanning = currentUser.role === 'secretaire' ||
    currentUser.role === 'medecin' ||
    currentUser.role === 'superviseur_qhse' ||
    currentUser.role === 'superadmin' ||
    filteredBookings.length > 0;

  const sectionOrder: Array<'security' | 'maintenance' | 'technical' | 'biomedical' | 'planning'> =
    currentUser.role === 'biomedical'
      ? ['biomedical', 'technical', 'security', 'maintenance', 'planning']
      : currentUser.role === 'technicien' || currentUser.role === 'superviseur_technicien'
        ? ['technical', 'biomedical', 'security', 'maintenance', 'planning']
        : currentUser.role === 'agent_entretien' || currentUser.role === 'superviseur_agent_entretien'
          ? ['maintenance', 'security', 'technical', 'biomedical', 'planning']
          : currentUser.role === 'agent_securite' || currentUser.role === 'superviseur_agent_securite'
            ? ['security', 'maintenance', 'technical', 'biomedical', 'planning']
            : ['security', 'maintenance', 'technical', 'biomedical', 'planning'];

  const anySectionVisible = showSecurity || showMaintenance || showTechnical || showBiomedical || showPlanning;
      ...
      {!anySectionVisible && (
        <Card>
          <CardContent className="p-6 text-sm text-muted-foreground">
            Aucune donnée KPI disponible pour le moment.
          </CardContent>
        </Card>
      )}

      {sectionOrder.map((section) => {
        if (section === 'security' && showSecurity) {
          return (
            <section key="security">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <Card>
                  ...
                  <CardContent className="h-64">
                    {securityIncidentsByPriority.length === 0 ? (
                      <EmptyChart />
                    ) : (
                      <ResponsiveContainer width="100%" height="100%">
        ...
        if (section === 'maintenance' && showMaintenance) {
          return (
        ...
        if (section === 'technical' && showTechnical) {
          return (
            <section key="technical">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <Card>
                  ...
                  <CardContent className="h-64">
                    {technicalIncidentsByStatus.length === 0 ? (
                      <EmptyChart />
                    ) : (
                      <ResponsiveContainer width="100%" height="100%">
        ...
        if (section === 'biomedical' && showBiomedical) {
          return (
            <section key="biomedical">
              <h3 className="text-2xl font-semibold mb-4 flex items-center">
        ...
        if (section === 'planning' && showPlanning) {
          return (
